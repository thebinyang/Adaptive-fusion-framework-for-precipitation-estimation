...
# ##构建经纬度网格
# lon_net=np.arange(69.95,140.05,0.01)
# lat_net=np.arange(14.95,55.05,0.01)
...
#     if table.shape[0]>0:
#         if mat[0,1]>=14.95 and mat[0,1]<=55.05 and mat[0,2]>=69.95 and mat[0,2]<=140.05:
...
#                 scipy.io.savemat('./GHCN-Daily-CR-001/record_4011_7011/'+str(lon_id)+'_'+str(lat_id)+'_'+str(mat[0,2])+'_'+str(mat[0,1])+'.mat', {'data': matrix})
...
files=os.listdir('./GHCN-Daily-CR-001/record_4011_7011/')

result=np.zeros((1,8),'float32')
...
for i in range(len(files)):
    ...
    ###地面站点数据
    load_mat = scipy.io.loadmat('./GHCN-Daily-CR-001/record_4011_7011/'+files[i])
    data_insitu = load_mat['data']
    data_insitu=data_insitu[0,:]
    print(data_insitu)
    
    ###Ours
    load_mat = scipy.io.loadmat('I:/水文+SR/新建文件夹/output/IMERG-Early_UP_output/'+files[i])
    data_imerg_early = load_mat['sequence']
    data_imerg_early=data_imerg_early[0,:]
    data_imerg_early[data_imerg_early<-1]=-1
    
    ###SM2RAIN_SR_output
    load_mat = scipy.io.loadmat('I:/水文+SR/新建文件夹/output/SM2RAIN_UP_output/'+files[i])##################################
    data_SM2RAIN_SR = load_mat['sequence']
    data_SM2RAIN_SR=data_SM2RAIN_SR[0,:]
    data_SM2RAIN_SR[data_SM2RAIN_SR<-1]=-1
    
    
    load_mat = scipy.io.loadmat('I:/水文+SR/新建文件夹/output/IMAGE-Early_SM2RAIN_UP_output/'+files[i])##################################
    data_IS_fusion = load_mat['sequence']
    data_IS_fusion=data_IS_fusion[0,:]
    data_IS_fusion[data_IS_fusion<-1]=-1
    
    #######分析
    matrix=np.zeros((1825,4),float)-1
    matrix[:,0]=data_insitu[:]
    matrix[:,1]=data_imerg_early[:]
    matrix[:,2]=data_SM2RAIN_SR[:]
    matrix[:,3]=data_IS_fusion[:]
    ...
    this_result=np.zeros((a.shape[0],8),'float32')
    this_result[:,0]=i
...
title=['IMAGE-Early_0.01', 'SM2RAIN_UP_0.01', 'IS_fusion_UP_0.01']##################################
year_r_matrix=np.zeros((3,5))
...
for year in range(2015, 2020):
    ...
    index = np.where((result[:, 4] > 0) & (result[:, 5] > 0) & (result[:, 6] > 0)& (result[:, 7] > 0))[0]

    result = result[index, :]
    r_imerg_early = np.corrcoef(result[:, 4], result[:, 5])[0, 1]
    r_SM2RAIN_SR = np.corrcoef(result[:, 4], result[:, 6])[0, 1]
    r_IS_fusion = np.corrcoef(result[:, 4], result[:, 7])[0, 1]
    r = [r_imerg_early, r_SM2RAIN_SR, r_IS_fusion]
...
# 计算绝对偏差
bias = result[:, :].copy()
bias[:, 5] = (bias[:, 5] - bias[:, 4])  # Model 1 Bias
bias[:, 6] = (bias[:, 6] - bias[:, 4])  # Model 2 Bias
bias[:, 7] = (bias[:, 7] - bias[:, 4])  # Model 2 Bias

# 计算真实值的总和
total_true_values = np.sum(result[:, 4])  # 取真实值列的总和

# 计算相对偏差
relative_bias = np.sum(bias[:, 5:], axis=0) / total_true_values  # 计算相对偏差（相对Bias）
...
for v in range(5, 8):
    ...
    plt.title(title[v - 5] + ' R=' + str(np.round(r[v - 5], 3)) + 
              ' Relative Bias=' + str(np.round(relative_bias[v - 5], 3)) + 
...
r_station=np.zeros((len(stations),14))
for s in range(len(stations)):
    ...
    r_station[s,7]=np.sqrt(np.sum((result[s_index,4]-result[s_index,5])**2)/len(result[s_index,4]))
    r_station[s,8]=np.sqrt(np.sum((result[s_index,4]-result[s_index,6])**2)/len(result[s_index,4]))
    r_station[s,9]=np.sqrt(np.sum((result[s_index,4]-result[s_index,7])**2)/len(result[s_index,4]))
...
for v in range(7, 10):
    ...
    plt.title(title[v - 7], fontsize=20, fontname='Times New Roman')
    plt.xticks(x_positions, x_labels, fontsize=20, fontname='Times New Roman')  # 更改x轴标签
    plt.yticks(y_positions, y_labels, fontsize=20, fontname='Times New Roman')  # 更改y轴标签
    plt.xlim(70, 140)  # 根据实际经度范围进行调整
    plt.ylim(15, 55)   # 根据实际纬度范围进行调整
    
    plt.savefig('./figure/RMSE_station/' + title[v - 7] + '.png')
    plt.close()
...
index=np.where((result[:,3]>=year_start)&(result[:,3]<year_end))[0]
result=result[index,:].copy()
index=np.where((result[:,4]>0)&(result[:,5]>0)&(result[:,6]>0))[0]
